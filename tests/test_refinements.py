...
def test_refinements_telemetry(otr_bench: Any, profile_name: str) -> None:
    ...
    def inputs_factory(shapes: Dict[str, Any]) -> Tuple[nn.Module, torch.Tensor]:
        ...
        x = torch.randn(shapes["B"], shapes["T"], shapes["C"], device=device, requires_grad=True)
        return (model, x)
    ...
    assert metrics["status"] in ["GEN_0_INITIALIZED", "PROMOTED", "REJECTED", "IMPROVED_INTRA"]
...
def test_test_only_and_purging() -> None:
    ...
    stale_small_found = any(t.get("run_name") == "stale_run_small" for t in final_workspace)
    ...
    workspace_data = state_mgr._load(state_mgr.workspace_path, list)
    workspace_data = [t for t in workspace_data if t.get("component") != "Test Component Purging"]
    state_mgr._save(state_mgr.workspace_path, workspace_data)
